...
namespace :cordova do
  ...
  task :install do
    sh %{ yarn list --depth 1 --global cordova; if [ $? -ne 0 ]; then yarn add global cordova; fi }
    sh %{ yarn list --depth 1 --global cordova-update-config; if [ $? -ne 0 ]; then yarn add global cordova-update-config; fi }
  end
