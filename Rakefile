...
namespace :cordova do
  ...
  task :install do
    sh %{ yarn list --depth 1 --global cordova; if [ $? -ne 0 ]; then npm install -g cordova; fi }
    sh %{ yarn list --depth 1 --global cordova-update-config; if [ $? -ne 0 ]; then npm install -g cordova-update-config; fi }
  end
